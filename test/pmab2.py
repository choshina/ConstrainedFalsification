...
for ph in phi_str:
	for cp in controlpoints:
		for opt in optimization:
			for ctr in cons:
				property = ph.split(';')
				filename = model+ '_breach_' + property[0]+'_' + str(cp)  +'_' + opt + '_' + ctr + '_' + 'mab'
				param = '\n'.join(parameters)
				#print pattern
				pat_s = pattern.split(';')
				with open('benchmarks/'+filename,'w') as bm:
					...
					#bm.write('cons = STL_Formula(\'cons\', \'' + cons + '\');\n')

					bm.write('consfile = \'' +ctr+ '\';\n')
					bm.write('cons = parse(consfile);\n')
...
#					bm.write('base = ' + bs + ';\n')
				...
				#	bm.write('\tfalsif_pb = PracticalFP_2(Br,phi, base);\n')
					bm.write('\tfalsif_pb = PFPMAB2(Br,phi,cons, budget, b_u, c, pattern);\n')
					if timeout!='':
...
#					bm.write('\tobj_best = [obj_best;falsif_pb.obj_best];\n')
					...
					for j in range(1,n_trials):
						bm.write(';filename')
					bm.write('};\n')
					#bm.write('base = base*ones(trials,1);\n')

					bm.write('consfile = {consfile')
					for j in range(1,n_trials):
						bm.write(';consfile')
					bm.write('};\n')
					bm.write('result = table(filename, spec, consfile , falsified, time);\n')
