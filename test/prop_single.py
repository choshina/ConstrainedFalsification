	...
	for cp in controlpoints:
		for opt in optimization:
			for ctr in cons:
				for s in S:
					property = ph.split(';')
					filename = model+ '_breach_' + property[0]+'_' + str(cp)  +'_' + opt + '_' + ctr + '_' + s +'_'+ 'single'
					param = '\n'.join(parameters)
...
#					bm.write('base = ' + bs + ';\n')
				...
				#	bm.write('\tfalsif_pb = PracticalFP_2(Br,phi, base);\n')
						bm.write('\tfalsif_pb = PracticalFP_Single(Br,phi,cons, S);\n')
						if timeout!='':
					...
					#bm.write('base = base*ones(trials,1);\n')
						...
						bm.write('S_str = {S_str')
						for j in range(1, n_trials):
							bm.write(';S_str')
						bm.write('};\n')

						bm.write('result = table(filename, spec, consfile  , falsified, time,  obj_best, num_sim, S_str);\n')
